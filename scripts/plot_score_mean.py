...
if __name__ == "__main__":
    ...
    for i, level in enumerate(levels):
        ...
        for j, model in enumerate(models):
            y_values = load_sweep_data(model, level, steps, "score_mean")
        ...
        if i == 0:
            ax.set_ylabel("Mean Orders Served", fontsize=12)

        # Add a buffer below 0 and above 3 so points don't sit on the border
        ax.set_ylim(-0.1, 3.2)
    ...
    plt.savefig("analytics/score_mean_over_checkpoints.png", dpi=300, bbox_inches='tight')
    print("Saved score_mean_over_checkpoints.png")