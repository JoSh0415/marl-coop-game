...
class GymCoopEnvRLlibDecentralisedComms(MultiAgentEnv):
    _instance_counter = 0
    ...
    def __init__(self, config=None):
        ...
        self.level_name = config.get("level_name", "level_2")
        ...
        if vector_index is None or int(vector_index) < 0:
            vector_index = GymCoopEnvRLlibDecentralisedComms._instance_counter
            GymCoopEnvRLlibDecentralisedComms._instance_counter += 1
    ...
    def step(self, action_dict):
        a1 = self._parse_env_action(action_dict.get("agent_1", 0))
        a2 = self._parse_env_action(action_dict.get("agent_2", 0))
    ...
    # Function: Parse environment action safely for backward compatibility
    def _parse_env_action(self, action):
